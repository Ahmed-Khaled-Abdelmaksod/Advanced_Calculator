...
void KPD_Init(void){
	...
	DIO_enumConnectPullup        ( KPD_PORT , KPD_R0 , DIO_HIGH );
	DIO_enumConnectPullup        ( KPD_PORT , KPD_R1 , DIO_HIGH );
	DIO_enumConnectPullup        ( KPD_PORT , KPD_R2 , DIO_HIGH );
	DIO_enumConnectPullup        ( KPD_PORT , KPD_R3 , DIO_HIGH );
...
u8   KPD_u8GetPressed( void ){
	...
	for( LOC_u8Col = 0 + KPD_COL_INIT ; LOC_u8Col < KPD_COL_END + 1 ; LOC_u8Col++ ){
		...
		DIO_enumSetPinValue    ( KPD_PORT , LOC_u8Col , DIO_LOW );   // Low on the columns
		...
		for( LOC_u8Row = 0 +KPD_ROW_INIT ; LOC_u8Row < KPD_ROW_END + 1 ; LOC_u8Row++ ){
			...
			if( LOC_u8GetPressed == 0 )
			{
				...
				while( DIO_LOW == LOC_u8GetPressed ) /*  This cond for safty instead of (LOC_u8GetPressed == DIO_PIN_LOW) if i foget = */
				{
